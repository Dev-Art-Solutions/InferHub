...
public sealed class NodeOptionsValidator : IValidateOptions<NodeOptions>
{
    public ValidateOptionsResult Validate(string? name, NodeOptions options)
    {
        ...
        foreach (var disabled in options.Capabilities.Disabled)
        {
            ...
            {
                failures.Add(
                    $"{NodeOptions.SectionName}:Capabilities:Disabled contains '{disabled}', which is not a capability this release knows. Expected one of: {CapabilityKinds.Chat}, {CapabilityKinds.Embed}, {CapabilityKinds.Transcribe}, {CapabilityKinds.Speak}, {CapabilityKinds.Image}, {CapabilityKinds.ImageEdit}, {CapabilityKinds.Rerank}.");
            }
        }
