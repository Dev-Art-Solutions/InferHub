...
- **`CorpusFailoverService`'s own interval/grace-period timer was not run end to end against a real
  two-node mesh.** Only `NodeCorpusReplicator.PromoteAsync` (what the timer calls once grace elapses)
  was exercised that way. The timer's own startup and config wiring **were** confirmed on the
  published image (below) — it really starts and reads `CorpusFailover:*` from the environment — but
  no real node ever went unreachable long enough for it to fire a promotion inside a container.
- **A real two-node, two-container standby-and-kill sequence was not run.** The published-image check
  below is a solo-coordinator smoke test (no node attached) confirming the new admin surface and
  service actually ship in the image, not a full failover drill with two real node containers and a
  real `docker kill` on the primary.
...
**The published image was pulled and run**
(`ghcr.io/dev-art-solutions/inferhub-coordinator:3.42.0`, digest
`sha256:225c975a2e97700177207272c7e2c52203d3a1c9583787966b285cfc14d24e08`, label
`org.opencontainers.image.revision` = `ef39611`, the release-ritual commit) — a solo coordinator
container, no node attached. `CorpusFailover:Enabled=true`/`CorpusFailover:GraceMinutes=1` set via
environment produced the real startup log line `Corpus failover starting: interval 30s, grace 1m`,
confirming `CorpusFailoverService` actually ships in the image and reads its config, not just that
the unit tests construct it. `POST /api/admin/collections/no-such-collection/standby/node-x` against
a name the hub owns (nothing here is node-owned) returned the real `409` refusal —
`"collection 'no-such-collection' is owned by hub; a standby only makes sense for a node-owned
collection"` — matching `CollectionOwnership.AssignStandby` exactly. `DELETE .../standby` on the same
name is a harmless `200` no-op. `GET /api/status` reports `"coordinatorVersion":"3.42.0"`. This is a
solo-container smoke test of the new admin surface and service, not the two-node failover drill named
above as still open.
